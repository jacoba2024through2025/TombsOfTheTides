...
@onready var spinning_sphere: GPUParticles3D = $"../SpinningSphere"
@onready var curse: AudioStreamPlayer3D = $"../curse"
...
var health_regen_rate = 0.8 # Health restored per second
...
func _on_particle_detection_body_entered(body: Node3D) -> void:
	...
	if body.name == "player":
		curse.play()
		Global.globalMusicStart = true
		var mesh := spinning_sphere.draw_pass_1
		if mesh:
			var mesh_material = mesh.surface_get_material(0)
			
			if mesh_material is StandardMaterial3D:
				var unique_material := mesh_material.duplicate() as StandardMaterial3D
				
				unique_material.set("albedo_color", Color(1, 0, 0))
